  ...
  return (
    <div style={{marginBottom:"5vh"}}>
      {women.map((woman, index) => (
        <section key={index} style={{backgroundColor: 'aqua', boxShadow: '15px 15px 15px #A582AF', display: 'flex', alignItems: 'center', height: '10vw', width: '75vw', marginLeft: '5vw', marginTop: '3vw', borderRadius: '1vw', position: 'relative'}}>
          ...
          <button style={{position: 'absolute', bottom: '10px', right: '10px', padding: '10px', backgroundColor: '#DAA7DB', color: '#fff', boxShadow: '5px 5px #A582AF', fontWeight:'bold', border: 'none', borderRadius: '5px', cursor: 'pointer',}} onClick={() => showDetails(woman)}>ver más</button>
        </section>
      ))}

      {selectedWoman && (
        <div style={{position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", backgroundColor: "white", width: "50%", border: "5px solid #DAA7DB", borderRadius: "10px", padding: "20px", boxShadow: "5px 5px 15px rgba(0, 0, 0, 0.2)", zIndex: "1"}}>
          <span style={{color: "#DAA7DB", fontWeight: "bold", fontSize: "2rem", marginBottom: "1rem"}}>{selectedWoman.name}</span>
          <p style={{textAlign: "justify"}}>{selectedWoman.description}</p>
          <button style={{backgroundColor: "#DAA7DB", color: "#fff", border: "none", borderRadius: "5px", padding: "10px", marginRight: "1rem", marginTop: "1rem", cursor: "pointer", float: "right"}} onClick={hideDetails}>Cerrar</button>
        </div>
      )}
    </div>
  );
...
export default LearnSpanishWoman;
